...
export const attachProfileListener = async (id: number) => {
	...
	if (user.avatar) (<HTMLImageElement>document.getElementById('avatar')).src = user.avatar;
	document.getElementById('nickname').textContent = user.nickname;

	const status = (await (await fetch(`/api/users/${userId}/ping`, { method: 'GET' })).json()).message;
	const statusDot = document.getElementById('statusDot');
	if (status === 'Online') statusDot.classList.add('bg-green-600');
	else if (status === 'Offline') statusDot.classList.add('bg-gray-500');
	else statusDot.classList.add('bg-orange-400');
