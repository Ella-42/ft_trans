...
export const attachFriendsListener = async () => {
    ...
    try {
        ...
        let online = 0;
        enrichedFriendsList.forEach(user => { if (user.ping === 'Online')
            online++; });
        let searchText = '';
        let userArray = [];
...
export const renderFriends = (userArray, searchText = '', userId, enrichedFriendsList, enrichedFriendRequestsArray, online) => {
    return `
