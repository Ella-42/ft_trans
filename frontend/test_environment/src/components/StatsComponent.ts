...
export const attachStatsListener= async (page = 1) => {
	...
	try {
		...
		const scoreData = matchHistoryArray.map(match => {
		...
		if (container) {
			...
			const maxDiff = Math.max(...normalizedDiffs);
			...
			for (let i = 0; i <= tickCount; i++) {
				...
				ctx.fillText(diffVal.toFixed(1), 10, y + 4);
			}
...
export const renderStats = (userId: number, totalCount: number, wins: number, losses: number, matchHistoryArray: Array<{time: string, winner: { id: number; nickname: string }, loser: {id: number; nickname: string}, info: string}>, currentPage: number, totalPages: number) => {
	return `
					...
					<div class="px-10 py-5 rounded-xl my-5 mb-10 bg-gray-900 flex flex-col justify-between">
						...
						<div class="grid grid-cols-1 md:grid-cols-4 mt-6 gap-4">
							...
							<div class="py-6 px-8 bg-slate-800 rounded-md">
								...
								<p class="text-2xl font-bold text-white">${losses ? (wins / losses).toFixed(2) : '∞'}</p>
							</div>
						</div>
