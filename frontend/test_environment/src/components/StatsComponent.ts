...
const getHorizontalTicks = (total) => {
	...
	if (total <= maxTicks) return total;
...
export const drawGraph = (scoreData, elementId) => {
	...
	if (total < 2) return canvas.replaceWith('too little data...');
	...
	if (maxDiff === minDiff) maxDiff = minDiff + 1;
...
const renderMatchPages = (userId: number, matchHistoryArray: Array<{time: string, winner: { id: number; nickname: string }, loser: {id: number; nickname: string}, info: string}>, currentPage: number, totalPages: number) => {
	return `
				<div class="flex flex-col items-center mt-6 text-center">
